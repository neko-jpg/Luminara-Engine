...
impl WasmScriptRuntime {
    pub fn new(limits: ResourceLimits) -> Result<Self, ScriptError> {
        ...
        let mut store = Store::new(&engine, HostState { limits: limits.clone(), instruction_count: 0 });

        if limits.max_instructions > 0 {
            store.set_fuel(limits.max_instructions).map_err(|e| ScriptError::Runtime(e.to_string()))?;
        }
        ...
        // Transform API
        linker.func_wrap("host", "get_position", |caller: wasmtime::Caller<'_, HostState>, entity_id: u64| -> (f32, f32, f32) {
        ...
        linker.func_wrap("host", "set_position", |caller: wasmtime::Caller<'_, HostState>, entity_id: u64, x: f32, y: f32, z: f32| {
        ...
        linker.func_wrap("host", "log", |mut caller: wasmtime::Caller<'_, HostState>, ptr: i32, len: i32| {
    ...
    pub fn load_module(&mut self, bytes: &[u8]) -> Result<ScriptId, ScriptError> {
        ...
        let instance = self.linker.instantiate(&mut self.store, &module)
            .map_err(|e| ScriptError::Runtime(format!("Failed to instantiate WASM module: {}", e)))?;

        let id = ScriptId(self.next_id);
        self.next_id += 1;

        self.modules.insert(id, WasmModule {
            id,
            instance,
        });
    ...
    pub fn write_to_memory(&mut self, script_id: ScriptId, data: &[u8]) -> Result<(i32, i32), ScriptError> {
        let module = self.modules.get(&script_id).ok_or_else(|| ScriptError::Runtime("Module not found".into()))?;
        let instance = module.instance;

        let alloc = instance.get_typed_func::<i32, i32>(&mut self.store, "alloc")
    ...
    pub fn read_from_memory(&mut self, script_id: ScriptId, ptr: i32, len: i32) -> Result<Vec<u8>, ScriptError> {
        let module = self.modules.get(&script_id).ok_or_else(|| ScriptError::Runtime("Module not found".into()))?;
        let instance = module.instance;
    ...
    pub fn call_json_func<Args: Serialize, Ret: for<'de> Deserialize<'de>>(
        ...
        if self.store.data().limits.max_instructions > 0 {
             self.store.set_fuel(self.store.data().limits.max_instructions).unwrap();
        }
        self.store.set_epoch_deadline(1);

        let json_bytes = serde_json::to_vec(&args).map_err(|e| ScriptError::Runtime(e.to_string()))?;
        let (ptr, len) = self.write_to_memory(script_id, &json_bytes)?;

        let module = self.modules.get(&script_id).ok_or_else(|| ScriptError::Runtime("Module not found".into()))?;
        let func = module.instance.get_typed_func::<(i32, i32), (i32, i32)>(&mut self.store, func_name)
            .map_err(|_| ScriptError::Runtime(format!("Function {} not found or signature mismatch", func_name)))?;

        let (ret_ptr, ret_len) = func.call(&mut self.store, (ptr, len))
            .map_err(|e| ScriptError::Runtime(format!("Call failed: {}", e)))?;

        let ret_bytes = self.read_from_memory(script_id, ret_ptr, ret_len)?;

        let ret: Ret = serde_json::from_slice(&ret_bytes).map_err(|e| ScriptError::Runtime(e.to_string()))?;
