...
use luminara_math::foundations::{incircle, orient3d, Expansion};
...
fn bench_predicates(c: &mut Criterion) {
    ...
    group.bench_function("orient3d_simple", |b| {
    ...
    group.bench_function("incircle_simple", |b| {
        b.iter(|| {
            incircle(
                black_box(p2a),
                black_box(p2b),
...
fn bench_motor(c: &mut Criterion) {
    ...
    group.bench_function("geometric_product", |b| {
        b.iter(|| black_box(m1).geometric_product(black_box(&m2)))
    });
    ...
    group.bench_function("mat4_mul", |b| {
        b.iter(|| black_box(mat1).mul_mat4(black_box(&mat2)))
    });
...
fn bench_bvh(c: &mut Criterion) {
    ...
    let count = 100_000;
    let primitives: Vec<BenchmarkPrim> = (0..count)
        .map(|i| {
            let x = (i % 100) as f32;
    ...
    group.bench_function("build_100k_parallel", |b| {
        b.iter(|| Bvh::build(black_box(primitives.clone())))
    });
...
fn bench_integrator(c: &mut Criterion) {
    ...
    group.bench_function("mk4_step", |b| {
        b.iter(|| LieGroupIntegrator::step(black_box(m), black_box(dt), |_| v))
    });

    group.bench_function("symplectic_step", |b| {
        b.iter(|| {
            SymplecticEuler::step(black_box(m), black_box(v), black_box(dt), |_| {
                Bivector::ZERO
            })
        })
    });
