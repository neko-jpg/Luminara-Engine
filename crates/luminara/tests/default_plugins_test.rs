...
fn test_default_plugins_bundle_initialization() {
    ...
    assert!(
        plugin_order.len() >= 9,
        "DefaultPlugins should register at least 9 plugins"
    );

    // Verify specific plugins are present
    assert!(
        app.has_plugin("PlatformPlugin"),
        "PlatformPlugin should be registered"
    );
    assert!(
        app.has_plugin("DiagnosticPlugin"),
        "DiagnosticPlugin should be registered"
    );
    assert!(
        app.has_plugin("WindowPlugin"),
        "WindowPlugin should be registered"
    );
    assert!(
        app.has_plugin("InputPlugin"),
        "InputPlugin should be registered"
    );
    assert!(
        app.has_plugin("ScenePlugin"),
        "ScenePlugin should be registered"
    );
    assert!(
        app.has_plugin("AssetPlugin"),
        "AssetPlugin should be registered"
    );
    assert!(
        app.has_plugin("RenderPlugin"),
        "RenderPlugin should be registered"
    );
    assert!(
        app.has_plugin("PhysicsPlugin"),
        "PhysicsPlugin should be registered"
    );
    assert!(
        app.has_plugin("AudioPlugin"),
        "AudioPlugin should be registered"
    );
}
...
fn test_default_plugins_initialization_order() {
    ...
    assert!(
        scene_idx < render_idx,
        "ScenePlugin should be initialized before RenderPlugin"
    );
    assert!(
        scene_idx < physics_idx,
        "ScenePlugin should be initialized before PhysicsPlugin"
    );
    assert!(
        asset_idx < render_idx,
        "AssetPlugin should be initialized before RenderPlugin"
    );
    assert!(
        asset_idx < audio_idx,
        "AssetPlugin should be initialized before AudioPlugin"
    );
}
...
fn test_default_plugins_not_duplicated() {
    ...
    let physics_count = plugin_order
        .iter()
        .filter(|p| *p == "PhysicsPlugin")
        .count();
    ...
    assert_eq!(
        render_count, 1,
        "RenderPlugin should be registered only once"
    );
    assert_eq!(
        physics_count, 1,
        "PhysicsPlugin should be registered only once"
    );
