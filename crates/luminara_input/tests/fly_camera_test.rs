...
impl FlyCameraController {
    ...
    pub fn move_forward(&mut self, delta_time: f32, sprint: bool) {
        let speed = if sprint { self.move_speed * self.sprint_multiplier } else { self.move_speed };
...
fn test_fly_camera_wasd_movement() {
    ...
    assert_ne!(camera.position(), initial_pos, "Camera should move forward with W key");
}
...
fn test_fly_camera_mouse_look() {
    ...
    assert_ne!(camera.yaw, 0.0, "Camera yaw should change with mouse movement");
    assert_ne!(camera.pitch, 0.0, "Camera pitch should change with mouse movement");
}
...
fn test_fly_camera_smooth_movement() {
    ...
    assert!(vel_magnitude > 0.0, "Camera should have smooth movement with inertia");
}
...
mod fly_camera_integration_tests {
    ...
    fn test_fly_camera_collision_detection() {
        ...
        assert_eq!(pos, Vec3::ZERO, "Camera position should be accessible for collision detection");
    }
    ...
    fn test_fly_camera_vertical_movement() {
        ...
        assert!(camera.position().y > initial_y, "Camera should move up with Space key");
        ...
        assert!(camera.position().y < after_up_y, "Camera should move down with Shift key");
    }
    ...
    fn test_fly_camera_sprint_mode() {
        ...
        assert!(sprint_velocity > normal_velocity, "Sprint mode should move faster");
    }
    ...
    fn test_fly_camera_inertia() {
        ...
        assert!(high_inertia_vel > low_inertia_vel, "Higher inertia should maintain velocity longer");
    }
...
mod fly_camera_math_tests {
    ...
    fn test_camera_forward_vector() {
        ...
        assert!(forward.length() > 0.99 && forward.length() < 1.01, "Forward vector should be normalized");
        
        // Rotate and check forward vector changes
        camera.update_rotation(90.0, 0.0);
        let rotated_forward = camera.forward();
        assert_ne!(forward, rotated_forward, "Forward vector should change with rotation");
    }
    ...
    fn test_camera_right_vector() {
        ...
        assert!(right.length() > 0.99 && right.length() < 1.01, "Right vector should be normalized");
        
        // Right vector should be perpendicular to forward
        let forward = camera.forward();
        let dot = forward.dot(right);
        assert!(dot.abs() < 0.01, "Right vector should be perpendicular to forward");
    }
    ...
    fn test_camera_up_vector() {
        ...
        let up = camera.up();
        assert_eq!(up, Vec3::new(0.0, 1.0, 0.0), "Up vector should be world up (0, 1, 0)");
    }
    ...
    fn test_pitch_clamping() {
        ...
        assert!(camera.pitch >= -89.0 && camera.pitch <= 89.0, "Pitch should be clamped to prevent gimbal lock");
    }
    ...
    fn test_orthogonal_basis() {
        ...
        // Check orthogonality
        assert!(forward.dot(right).abs() < 0.01, "Forward and right should be orthogonal");
        assert!(forward.dot(up).abs() < 0.5, "Forward and up should be roughly orthogonal");
        assert!(right.dot(up).abs() < 0.01, "Right and up should be orthogonal");
    }
}
