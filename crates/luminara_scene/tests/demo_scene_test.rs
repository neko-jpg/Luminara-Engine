...
fn test_phase1_demo_scene_loads() {
    ...
    assert!(
        ...
        "Sun should have DirectionalLight component"
    );
    ...
    assert!(
        ground.components.contains_key("PbrMaterial"),
        "Ground should have PbrMaterial component"
    );
    ...
    assert!(
        sphere.components.contains_key("PbrMaterial"),
        "Sphere should have PbrMaterial component"
    );
}
...
fn test_demo_scene_ron_format() {
    ...
    if let Ok(content) = ron_content {
        let scene = Scene::from_ron(&content).expect("Failed to parse demo scene from RON");
