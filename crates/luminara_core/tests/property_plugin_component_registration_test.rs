...
impl Plugin for ComponentRegisteringPlugin {
    fn build(&self, app: &mut App) {
        ...
        if self.register_resource2 {
            app.insert_resource(TestResource2 {
                name: "test".to_string(),
            });
        }
...
fn test_component_can_be_used_after_registration() {
    ...
    app.world
        .add_component(entity, TestComponent1 { value: 42 });
...
fn test_multiple_plugins_accumulate_registrations() {
    ...
    let plugin1 =
        ComponentRegisteringPlugin::new("Plugin1", true, false, false, true, false, false);
    app.add_plugins(plugin1);

    // Second plugin registers component 2 and resource 2
    let plugin2 =
        ComponentRegisteringPlugin::new("Plugin2", false, true, false, false, true, false);
    app.add_plugins(plugin2);

    // Third plugin registers component 3 and resource 3
    let plugin3 =
        ComponentRegisteringPlugin::new("Plugin3", false, false, true, false, false, true);
