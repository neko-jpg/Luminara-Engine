...
fn test_plugin_build_called_once() {
    ...
    assert_eq!(builds.len(), 1, "Plugin build should be called exactly once");
...
fn test_plugin_not_built_twice() {
    ...
    let builds = tracker.lock().unwrap();
    assert_eq!(builds.len(), 1, "Plugin build should be called only once even if registered twice");
}
...
fn test_has_plugin() {
    ...
    assert!(app.has_plugin("test_plugin"), "Should detect registered plugin");
    assert!(!app.has_plugin("nonexistent"), "Should not detect unregistered plugin");
}
...
fn test_empty_app_no_plugins() {
    ...
    assert_eq!(app.plugin_order().len(), 0, "New app should have no plugins");
    assert!(!app.has_plugin("any_plugin"), "New app should not have any plugins");
}

// Test for Requirements 9.2 and 9.3: Plugin system and resource registration

use luminara_core::{CoreStage, Resource, Query, impl_component};
...
fn test_plugin_can_register_components() {
    ...
    assert_eq!(components.len(), 1, "Component should be registered and usable");
...
fn test_multiple_plugins_register_different_components() {
    ...
    app.world.add_component(entity, ComponentB("test".to_string()));
