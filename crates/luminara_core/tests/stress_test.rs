...
fn stress_test_game_simulation() {
    ...
    move_access.components_write.insert(TypeId::of::<Position>());
    move_access.components_read.insert(TypeId::of::<Velocity>());

    let mut health_access = SystemAccess::default();
    health_access.components_write.insert(TypeId::of::<Health>());

    app.add_system(CoreStage::Update, IntoSystem::<WorldRefParam>::into_system(movement_system).with_access(move_access));
    app.add_system(CoreStage::Update, IntoSystem::<WorldRefParam>::into_system(health_system).with_access(health_access));
