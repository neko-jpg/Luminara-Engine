...
fn plugin_names_strategy() -> impl Strategy<Value = Vec<String>> {
    prop::collection::vec(plugin_name_strategy(), 2..10)
        ...
        .prop_filter("Must have at least two unique names", |names| names.len() >= 2)
}
...
fn test_two_plugins_execution_order() {
    ...
    assert_eq!(build_order[0], "FirstPlugin", "First plugin should be built first");
    assert_eq!(build_order[1], "SecondPlugin", "Second plugin should be built second");
}
...
fn test_plugin_order_matches_app_plugin_order() {
    ...
    for (i, name) in plugin_names.iter().enumerate() {
        assert_eq!(&build_order[i], name, "Build order should match at position {}", i);
        assert_eq!(&app_order[i], name, "App plugin_order should match at position {}", i);
    }
}
...
fn test_plugin_order_with_duplicate_registration() {
    ...
    let build_order = tracker.lock().unwrap();
    assert_eq!(build_order.len(), 3, "Duplicate plugin should not be built again");
