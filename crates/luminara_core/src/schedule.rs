...
impl Schedule {
    ...
    fn run_systems(systems: &mut [Box<dyn System>], world: &mut World) {
        ...
        for system in systems.iter_mut() {
            ...
            if Self::conflicts(&current_batch_access, &access) || (access.components_write.is_empty() && access.components_read.is_empty() && access.resources_write.is_empty() && access.resources_read.is_empty()) {
        ...
        for mut batch in batches {
            if batch.len() == 1 {
                ...
                let systems_ptrs: Vec<usize> = batch.iter().map(|s| *s as *const Box<dyn System> as usize).collect();
    ...
    fn conflicts(a: &SystemAccess, b: &SystemAccess) -> bool {
        if a.resources_write.iter().any(|id| b.resources_write.contains(id)) { return true; }
        if a.components_write.iter().any(|id| b.components_write.contains(id)) { return true; }
        if a.resources_read.iter().any(|id| b.resources_write.contains(id)) { return true; }
        if a.resources_write.iter().any(|id| b.resources_read.contains(id)) { return true; }
        if a.components_read.iter().any(|id| b.components_write.contains(id)) { return true; }
        if a.components_write.iter().any(|id| b.components_read.contains(id)) { return true; }
        false
    }
