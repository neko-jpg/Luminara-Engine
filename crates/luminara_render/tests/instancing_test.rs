...
fn test_instancing_reduces_draw_calls() {
    ...
    let mesh_handles: Vec<Handle<Mesh>> = (0..10)
        .map(|_| Handle::default())
        .collect();
...
fn test_target_1000_objects_10_meshes() {
    ...
    println!("✓ Target achieved: {} draw calls for {} objects ({}x instancing)",
...
fn test_worst_case_all_unique_meshes() {
    ...
    println!("⚠️  Worst case: {} draw calls for {} objects (no instancing benefit)",
...
fn test_batching_efficiency_metrics() {
    ...
    for (objects, draw_calls, description) in scenarios {
        ...
        println!("{}: {} objects, {} draw calls, {:.2}x instancing",
...
fn test_property_instancing_ratio_minimum() {
    ...
    for (objects, draw_calls) in test_cases {
        ...
        assert!(stats.instancing_ratio >= 1.0,
            "Instancing ratio should be at least 1.0, got {}", stats.instancing_ratio);
    }
}
...
fn test_property_draw_calls_bounded() {
    ...
    for (objects, draw_calls) in test_cases {
        assert!(draw_calls <= objects,
            "Draw calls ({}) should not exceed object count ({})", draw_calls, objects);
    }
}
