...
fn test_mesh_loader_with_valid_gltf() {
    ...
    assert!(result.is_ok(), "Failed to load valid GLB: {:?}", result.err());
    
    let mesh = result.unwrap();
    assert!(mesh.vertices.len() >= 3, "Mesh should have at least 3 vertices for a triangle");
    assert!(mesh.indices.len() >= 3, "Mesh should have at least 3 indices for a triangle");
