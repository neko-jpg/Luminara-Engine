...
fn perspective_params_strategy() -> impl Strategy<Value = (f32, f32, f32, f32)> {
    (fov_strategy(), near_plane_strategy(), aspect_ratio_strategy()).prop_flat_map(|(fov, near, aspect)| {
...
fn orthographic_params_strategy() -> impl Strategy<Value = (f32, f32, f32, f32)> {
    (ortho_size_strategy(), near_plane_strategy(), aspect_ratio_strategy()).prop_flat_map(|(size, near, aspect)| {
...
proptest! {
    ...
    fn prop_perspective_projection_produces_valid_matrix(
        ...
        if transformed_near.w.abs() > 1e-6 {
            ...
            prop_assert!(
                ndc_z >= -1.0 && ndc_z <= 1.0,
                "Transformed near point should be in NDC range [-1, 1].\n\
                 NDC Z: {}",
                ndc_z
            );
        }
    ...
    fn prop_orthographic_projection_produces_valid_matrix(
        ...
        prop_assert!(
            transformed.z >= -1.0 && transformed.z <= 1.0,
            "Transformed point should have z in NDC range [-1, 1].\n\
             Z: {}",
            transformed.z
        );
    }
