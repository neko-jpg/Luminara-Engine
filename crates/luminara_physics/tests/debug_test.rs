use luminara_render::{GizmoType, DrawCommand};
use luminara_physics::debug::{PhysicsDebugConfig, physics_debug_render_system};
...
use luminara_math::{Transform, Vec3, Color};
...
fn test_physics_debug_gizmos() {
    ...
    app.world.add_component(entity, Transform::from_translation(Vec3::new(1.0, 2.0, 3.0)));
    app.world.add_component(entity, Collider {
...
fn test_gizmo_type_categorization() {
    ...
    let box_shape = ColliderShape::Box { half_extents: Vec3::ONE };
    ...
    match box_shape {
        ColliderShape::Box { half_extents } => {
            let gizmo = GizmoType::Box { half_extents: [half_extents.x, half_extents.y, half_extents.z] };
