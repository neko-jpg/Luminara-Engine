...
fn test_handle_ron_serialization() {
    ...
    let deserialized: Handle<TestAsset> = ron::from_str(&ron_str).expect("Failed to deserialize Handle from RON");
...
fn test_asset_id_ron_serialization() {
    ...
    // Deserialize from RON
    let deserialized: AssetId = ron::from_str(&ron_str).expect("Failed to deserialize AssetId from RON");
