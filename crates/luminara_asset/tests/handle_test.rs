...
fn test_handle_ron_serialization() {
    ...
    let deserialized: Handle<TestAsset> =
        ron::from_str(&ron_string).expect("Failed to deserialize Handle from RON");
...
fn test_asset_id_ron_serialization() {
    ...
    let deserialized: AssetId =
        ron::from_str(&ron_string).expect("Failed to deserialize AssetId from RON");
...
fn test_asset_id_from_path_serialization() {
    ...
    // Test RON
    let ron_string =
        ron::to_string(&original).expect("Failed to serialize path-based AssetId to RON");
    let deserialized: AssetId =
        ron::from_str(&ron_string).expect("Failed to deserialize path-based AssetId from RON");
    assert_eq!(original, deserialized);

    // Test binary
    let binary =
        bincode::serialize(&original).expect("Failed to serialize path-based AssetId to binary");
    let deserialized: AssetId = bincode::deserialize(&binary)
        .expect("Failed to deserialize path-based AssetId from binary");
    assert_eq!(original, deserialized);
