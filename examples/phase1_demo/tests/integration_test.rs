...
fn test_scene_loads_without_errors() {
    ...
    if !scene_path.exists() {
        eprintln!("Skipping test: demo scene file not found at {:?}", scene_path);
        return;
    }
    ...
    assert_eq!(scene.entities.len(), 4, "Scene should have 4 entities (Camera, Sun, Ground, Sphere)");
    
    // Verify all required entities are present
    let entity_names: Vec<&str> = scene.entities.iter().map(|e| e.name.as_str()).collect();
    assert!(entity_names.contains(&"Camera"), "Scene should contain Camera entity");
    assert!(entity_names.contains(&"Sun"), "Scene should contain Sun entity");
    assert!(entity_names.contains(&"Ground"), "Scene should contain Ground entity");
    assert!(entity_names.contains(&"Sphere"), "Scene should contain Sphere entity");
    
    // Verify Camera has required components
    let camera = scene.entities.iter().find(|e| e.name == "Camera").unwrap();
    assert!(camera.components.contains_key("Transform"), "Camera should have Transform");
    ...
    assert!(sun.components.contains_key("DirectionalLight"), "Sun should have DirectionalLight");
    
    // Verify Ground has physics components
    let ground = scene.entities.iter().find(|e| e.name == "Ground").unwrap();
    assert!(ground.components.contains_key("Collider"), "Ground should have Collider");
    assert!(ground.components.contains_key("RigidBody"), "Ground should have RigidBody");
    
    // Verify Sphere has physics components
    let sphere = scene.entities.iter().find(|e| e.name == "Sphere").unwrap();
    assert!(sphere.components.contains_key("Collider"), "Sphere should have Collider");
    assert!(sphere.components.contains_key("RigidBody"), "Sphere should have RigidBody");
}
...
fn test_physics_simulation_runs() {
    ...
    assert!(app.world.get_resource::<luminara::physics::PhysicsWorld3D>().is_some(),
    ...
    assert!(world.get_component::<RigidBody>(ground).is_some(), 
        "Ground should have RigidBody component");
    assert!(world.get_component::<Collider>(ground).is_some(), 
        "Ground should have Collider component");
    assert!(world.get_component::<RigidBody>(sphere).is_some(), 
        "Sphere should have RigidBody component");
    assert!(world.get_component::<Collider>(sphere).is_some(), 
        "Sphere should have Collider component");
    
    // Verify rigid body types
    let ground_rb = world.get_component::<RigidBody>(ground).unwrap();
    assert!(matches!(ground_rb.body_type, RigidBodyType::Static), 
        "Ground should be static");
    
    let sphere_rb = world.get_component::<RigidBody>(sphere).unwrap();
    assert!(matches!(sphere_rb.body_type, RigidBodyType::Dynamic), 
...
fn test_rendering_pipeline_initializes() {
    ...
    world.add_component(light, luminara::render::DirectionalLight {
    ...
    world.add_component(mesh_entity, Transform {
        translation: Vec3::new(0.0, 1.0, 0.0),
        rotation: Quat::IDENTITY,
        scale: Vec3::ONE,
    });
    world.add_component(mesh_entity, GlobalTransform::default());
    world.add_component(mesh_entity, luminara::render::PbrMaterial {
    ...
    // Verify components were added successfully
    assert!(world.get_component::<Camera>(camera).is_some(), "Camera component should be added");
    assert!(world.get_component::<luminara::render::DirectionalLight>(light).is_some(), 
        "DirectionalLight component should be added");
    assert!(world.get_component::<luminara::render::PbrMaterial>(mesh_entity).is_some(), 
...
fn test_audio_system_initializes() {
    ...
    world.add_component(listener, Transform {
        translation: Vec3::new(0.0, 0.0, 0.0),
        rotation: Quat::IDENTITY,
        scale: Vec3::ONE,
    });
    ...
    world.add_component(audio_source, Transform {
        translation: Vec3::new(5.0, 0.0, 0.0),
        rotation: Quat::IDENTITY,
        scale: Vec3::ONE,
    });
    world.add_component(audio_source, GlobalTransform::default());
    world.add_component(audio_source, AudioSource {
    ...
    assert!(world.get_component::<AudioListener>(listener).is_some(), 
        "AudioListener component should be added");
    assert!(world.get_component::<AudioSource>(audio_source).is_some(), 
...
fn test_default_plugins_for_demo() {
    ...
    assert!(app.has_plugin("ScenePlugin"), "ScenePlugin required for scene loading");
    assert!(app.has_plugin("AssetPlugin"), "AssetPlugin required for asset loading");
    assert!(app.has_plugin("RenderPlugin"), "RenderPlugin required for rendering");
    assert!(app.has_plugin("PhysicsPlugin"), "PhysicsPlugin required for physics simulation");
    assert!(app.has_plugin("AudioPlugin"), "AudioPlugin required for audio playback");
    assert!(app.has_plugin("WindowPlugin"), "WindowPlugin required for display");
}
...
fn test_demo_runs_multiple_frames() {
    ...
    for frame in 0..60 {
        ...
        let transform = app.world.get_component::<Transform>(sphere)
            .expect(&format!("Sphere should exist at frame {}", frame));
        
        // Verify no NaN values
        assert!(transform.translation.is_finite(), 
            "Transform translation should be finite at frame {}", frame);
        assert!(transform.rotation.is_finite(), 
            "Transform rotation should be finite at frame {}", frame);
    }
}
