...
use luminara_render::{PointLight, PbrMaterial};
use luminara_physics::{RigidBody, Collider, ColliderShape, RigidBodyType};
...
fn add_cube_mesh(world: &mut World, entity: Entity, size: f32) {
    let mesh_h = world.get_resource::<AssetServer>().map(|a| a.add(Mesh::cube(size)));
    if let Some(h) = mesh_h { world.add_component(entity, h); }
}
...
fn add_sphere_mesh(world: &mut World, entity: Entity, radius: f32, segments: u32) {
    let mesh_h = world.get_resource::<AssetServer>().map(|a| a.add(Mesh::sphere(radius, segments)));
    if let Some(h) = mesh_h { world.add_component(entity, h); }
}
...
pub fn create_rotating_platforms(world: &mut World) {
    let platforms = [
        ...
        (Vec3::new(-15.0, 3.0, 10.0), Vec3::new(8.0, 0.5, 2.0), Vec3::Y, 0.5),
        (Vec3::new(-5.0, 5.0, 10.0), Vec3::new(6.0, 0.5, 2.0), Vec3::Y, -0.7),
        (Vec3::new(5.0, 7.0, 10.0), Vec3::new(8.0, 0.5, 2.0), Vec3::Y, 0.6),
        (Vec3::new(15.0, 9.0, 10.0), Vec3::new(6.0, 0.5, 2.0), Vec3::Y, -0.8),
        (Vec3::new(0.0, 11.0, 10.0), Vec3::new(10.0, 0.5, 2.0), Vec3::Y, 0.4),
    ];
...
pub fn create_magnetic_field_demo(world: &mut World) {
    ...
    for (i, (pos, attract, color)) in field_positions.iter().enumerate() {
        ...
        let name = if *attract { "MagneticAttractor" } else { "MagneticRepeller" };
